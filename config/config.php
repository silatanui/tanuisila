...
// Using PHP's getenv() which reads from $_ENV, $_SERVER, and .env file (if using vlucas/dotenv)
// For native PHP support, ensure .env file is accessible and use a package like vlucas/phpdotenv

// Load .env file if it exists (requires composer require vlucas/phpdotenv)
$envFile = __DIR__ . '/../.env';
...
define('ADMIN_KEY', $_ENV['ADMIN_KEY'] ?? 'change-me');
...
function ensurePortfolioSchema(PDO $pdo): void {
    ...
    $pdo->exec("INSERT INTO `about` (`id`, `content`) VALUES (1, 'Tanui Kipng\'etich Sila is a Computer Science graduate whose work sits where academic rigour meets practical software delivery. He com[...]
    $pdo->exec("INSERT INTO `profile` (`id`, `full_name`, `headline`, `bio`, `email`, `phone`, `location`, `website`, `linkedin`, `github`, `summary`) VALUES (1, 'Tanui Kipng\'etich Sila', 'MSc Comput[...]
    $pdo->exec("INSERT INTO `settings` (`setting_key`, `setting_value`) VALUES ('site_title', 'Tanui Kipng\'etich Sila'), ('tagline', 'MSc Computer Science Student at University of Debrecen'), ('hero_[...]
    $pdo->exec("INSERT INTO `contact` (`email`, `location`, `phone`) VALUES ('silatanuikipngetich@gmail.com', 'Debrecen, Hungary / Nairobi, Kenya', '+36 20 323 3673 / +254 742 178 644') ON DUPLICATE K[...]

    $pdo->exec("INSERT IGNORE INTO `skills` (`name`, `level`) VALUES ('Python', 'Advanced'), ('C#', 'Advanced'), ('C++', 'Intermediate'), ('JavaScript', 'Advanced'), ('PHP', 'Advanced'), ('MySQL', 'Ad[...]
    ...
    $projectRows = [
        ['AI-Driven E-Commerce Automation', 'Master\'s research project automating e-commerce workflows with AI technologies on the Blazor Server framework — intelligent product recommendations, inv[...]
        ['Global AI Impact Visualization', 'A dashboard presenting AI adoption metrics across 5000+ data points, with an interactive choropleth map and filtering to explore adoption rates and consumer[...]
        ['Interactive Curve Design System', 'A parametric curve design system implementing Bézier, Hermite and B-Spline algorithms with real-time differential geometry analysis.', 'https://tanuiksila[...]
        ['Post Correspondence Problem Solver', 'An interactive solver and visualization for the Post Correspondence Problem, a classic undecidable problem in formal language theory.', 'https://tanuiks[...]
    ];
    ...
    $pdo->exec("INSERT INTO `experience` (`company`, `role_name`, `location`, `start_date`, `end_date`, `description`, `sort_order`) VALUES
        ('Transcosmos (Fossil Group)', 'Customer Success Specialist | Enterprise Application & Customer Support', 'Debrecen, Hungary', '01/10/2025', '01/02/2026', '• Investigated customer, order, re[...]
        ('Infolink College of Technology', 'ICT and Programming Tutor', 'Nairobi, Kenya', '10/04/2023', '01/09/2024', '• Supported the development and maintenance of college website using PHP, JavaS[...]
        ('Litmed College of Technology', 'IT Technician & Lecturer', 'Nairobi, Kenya', '09/09/2022', '31/03/2023', '• Provided IT support and technical assistance to users, investigating issues and [...]
}
...
try {
    ...
    echo 'Database connection failed. Please check your .env file and verify MySQL is running.';
    exit;
}
