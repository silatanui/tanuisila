            ...
            <div class="timeline-desc" style="display: flex; gap: 20px; align-items: flex-start;">
              <?php if (!empty($uniLogo)): ?>
                <div class="img-skeleton-wrap" style="flex-shrink: 0; width: 64px; height: 64px; padding: 4px; background: var(--soft-bg); border: 1px solid var(--light-gray); display: flex; align-items: center; justify-content: center;">
                  <img 
                    src="<?php echo htmlspecialchars(appUrl('public/' . $uniLogo), ENT_QUOTES, 'UTF-8'); ?>?v=<?php echo @filemtime(__DIR__ . '/' . $uniLogo); ?>" 
                    alt="<?php echo htmlspecialchars($item['institution']); ?> Logo" 
                    loading="lazy"
                    decoding="async"
              ...
              <div style="flex-grow: 1;">
                ...
                <?php if (!empty($item['education_details']) || !empty($item['description'])): ?>
                  <div class="timeline-body" style="margin-top: 8px;"><?php echo nl2br(htmlspecialchars($item['education_details'] ?: $item['description'])); ?></div>
                <?php endif; ?>
              </div>
            </div>
          </div>
